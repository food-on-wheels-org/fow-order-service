    ...
    stages {
        ...
        stage('Code Coverage') {
            steps{
                script{
                    ...
                    def componentKey = "com.project.tejas:order-service"
        ...
        stage('Docker Build and Push') {
            steps {
                ...
                sh 'docker build -t tejassrivathsa/order-service:${VERSION} .'
                sh 'docker push tejassrivathsa/order-service:${VERSION}'
            }
        }
        ...
        stage('Update Image tag in GitOps') {
            steps {
                ...
                script {
                       sh '''
                          sed -i "s/image:.*/image: tejassrivathsa\\/order-service:${VERSION}/" k8s/apps/order-service-manifest.yml
                        '''
